...
for filename in glob.glob("hms-08-*.txt"):
...
for filename in glob.glob("hms-10-*.txt"):
...
for filename in glob.glob("hms-12-*.txt"):
...
plt.figure()
plt.title("Lattice hms under predominantly Z errors")
...
plt.xlabel("$p_z=10p_x=10p_{SPAM}$")
plt.savefig("hms.pdf")
plt.close()
